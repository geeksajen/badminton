...
# 三種角色與權限
1. **系統管理員**（Email 白名單，最高權限）：審核教練申請、可核准 / 拒絕 / 停權任何教練。
2. **教練**（動態身分，由系統管理員核准）：可建立課程、管理學員報名、確認繳費。可在個人專區申請成為教練、提交後等待審核。
3. **一般使用者**（預設）：登入後自動建立，可瀏覽課程、報名、在個人專區申請成為教練。

# 必要功能與網頁畫面
1. **導覽列（Navbar）**：顯示系統名稱、首頁連結、個人專區連結（登入後顯示）、教練後台連結（教練 / 系統管理員顯示）、角色徽章、登入/登出按鈕。
2. **首頁（課程列表）**：
...
4. **個人專區（Dashboard）**：
   ...
   - 「成為教練」申請卡片：依狀態顯示「申請成為教練」/ 「審核中（可撤回）」/ 「你已是教練 ✓」/ 「申請未通過」/ 「已停權」；系統管理員不顯示此卡片。
   - 報名課程列表，可「通知已匯款」或「取消報名」。
5. **教練後台（Admin / 教練專區，`/admin`）**：
   - 需為教練或系統管理員才能進入（採**前後端雙重把關**：前端 context 的 `isStaff`，Firestore Rules 的 `isStaff()` = `isSystemAdmin() || isCoach()`）。
   - **報名管理分頁**（教練 / 系統管理員）：一覽所有學員報名，可依狀態（進行中 / 待繳費 / 已確認 / 已取消）篩選與關鍵字搜尋；「確認收款 / 退回待繳費」一鍵更新狀態；上方統計卡片；「匯出 CSV」把目前篩選結果輸出成 UTF-8(BOM) 檔，Excel 可直接開。
   - **課程管理分頁**（教練 / 系統管理員）：直接在網頁新增 / 編輯 / 刪除課程，無需再進 Firebase 後台或跑 seed 腳本；含驗證（學費 ≥ 0、名額為正整數、編輯時名額上限不可低於目前已報名人數）。
   - **教練審核分頁**（**僅系統管理員**）：一覽所有教練申請，可依狀態（待審核 / 已核准 / 已停權 / 未通過）篩選；「核准」教練、「拒絕」（含拒絕理由）、「停權」已核准教練（含停權理由）；拒絕 / 停權的備註會顯示給申請人。
...
1. **Firebase 後端設定**：
   ...
   - 提供一份安全的 Firestore 安全規則（Security Rules），實作三角色權限模型。**規則內容以專案根目錄的 `firestore.rules` 為準**，每次更新後都要到 Firebase Console → Firestore → 規則重新貼上並發佈。
   - **設定系統管理員**：把管理員的登入 Email 同時填入 `src/config.js` 的 `SYSTEM_ADMIN_EMAILS` 與 `firestore.rules` 的 `isSystemAdmin()` 函式內，兩處需完全一致；改完要重新 `npm run deploy` 並重新發佈規則。
2. **GitHub Pages 前端部署**：
